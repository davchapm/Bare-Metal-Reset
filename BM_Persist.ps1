...
Write-Host This script will Power Down all Bare-Metal VMs, convert the disk mode to Persistent and Power-up the VMs
sleep 2
...
# Set all bare-metal servers to Persistent

$strVMs=@("BM1A","BM2A","BM3A","BM4A","BM5A","BM6A","BM7A","BM8A","BM1B","BM2B","BM3B","BM4B","BM5B","BM6B","BM7B","BM8B")
foreach ($objitem in $strVMs) {
Get-HardDisk -VM $objitem | Set-HardDisk -Persistence "IndependentPersistent" -Confirm:$false
}
...
# Disconnect from p9-vcenter.dc.local
